#ifndef TEST_LBFGS_HPP
#define TEST_LBFGS_HPP

#include <Eigen/Eigen>
#include <cmath>
#include <algorithm>

namespace test_lbfgs
{
    // ----------------------- Data Type Part -----------------------
    ...
    struct lbfgs_parameter_t
    {
         ...
         */
        int mem_size = 8;//用于存储前8次迭代的结果，用于近似计算H逆矩阵
         ...
         */
        double g_epsilon = 1.0e-5;//用于梯度收敛测试，终止条件之一
         ...
         */
        int past = 3;//决策变量的变化量收敛测试，终止条件之二
         ...
         */
        double delta = 1.0e-6;//函数值变化的收敛条件，终止条件之三
         ...
         *  is 0.
         */
        int max_iterations = 0;//最大迭代次数，设置为0不限制最大迭代次数，直到终止条件满足为止
         ...
         */
        int max_linesearch = 64;//线性搜索最大尝试次数
         ...
         *  be increased).
         */
        double min_step = 1.0e-20;//线搜最小步长
         ...
         *  be increased).
         */
        double max_step = 1.0e+20;//线搜最大步长
         ...
         */
        double f_dec_coeff = 1.0e-4;//线搜精度，控制的是直线的斜率，第一个wolf条件
         ...
         */
        double s_curv_coeff = 0.9;//线搜精度，控制的是切线斜率，第二个wolf条件
         ...
         */
        double cautious_factor = 1.0e-6;//全局收敛精度
